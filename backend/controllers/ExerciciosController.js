...
const getExerciciosNoLimit = async (req, res) => {
	try {
		const result = await pool.query(
			"SELECT * FROM exercises ORDER BY id_exercise"
		);
	...
	try {
		...
		if (!exercise_name || exercise_name.trim() === "") {
			return res.status(404).json({
				success: false,
				message: "Nome do exercício não pode estar vazio",
			});
		}
		...
		if (checkExistence.rows.length > 0) {
			return res
				.status(400)
				.json({ success: false, message: "Esse exercício já existe" });
		}
		...
		return res
			.status(201)
			.json({ success: true, message: "Exercício criado com sucesso" });
	} catch (error) {
...
const editExercicio = async (req, res) => {
	...
	try {
		...
		res
			.status(200)
			.json({ success: true, message: "Exercício editado com sucesso" });
	} catch (error) {
